...
Scope is deliberately tight: **five technical reference experiments (organized as four experiment families) across two domains (Finance, Economics), four forecasting paradigms, and three primary data sources (StatCan, FRED, yfinance)**. This is what we build, teach, and demo. Everything outside that scope is documented as out-of-scope or as learn-days discussion material.
...
| 3a | **Financial Markets — S&P 500 (primary template)** | Continuous, financial (Track 1 + Track 2) | yfinance | The with-futures case template: liquid-market forecasting with futures-aware covariate comparisons and anti-leakage backtesting. This is the primary convergence surface and the first build target for the financial-markets family. |
| 3b | **Financial Markets — Energy Commodities (extension of 3a)** | Continuous, financial (Track 1 + Track 2) | FRED, yfinance | Reuses the 3a experiment structure (same horizons, same core code) with WTI/RBOB data substitutions. This is intentionally scoped as a data-extension task, not a separate infrastructure build. |
| 4 | **Bank of Canada Rate Decisions** | Binary, decision-driven (Track 1) | StatCan, FRED | The sole binary-paradigm reference experiment. A sparse, publicly-documented decision process with clear resolution criteria. Drives the introduction of `BinaryForecast`, `BinaryPredictor`, and Brier scoring to the framework. |

**The convergence — bootcamp centrepiece.** Experiment 3a (S&P 500) is the primary Track 1 + Track 2 convergence surface. The same flagship agent that emits `ContinuousForecast` outputs for Track 1 backtesting is exercised on Track 2 task types — monitoring, scenario analysis, research Q&A, reasoning walkthroughs — on the same data. One agent, two modes. Experiment 3b (energy commodities) is the structurally identical extension (same horizons, same core code, different series) and is layered on once 3a is stable.

**Dependency structure.** Experiments 1–2 are complete as of Apr 2026. Experiment 3a (S&P 500 template) is in active development (Behnoosh); 3b (energy) extends it once the template is solid. Experiment 4 (BoC) introduces the `BinaryForecast` payload type and unlocks the binary paradigm for the whole framework — it is the most expensive experiment to build, but the return is the second paradigm. The flagship frontier agent is developed in parallel with experiment 3a, with the explicit intent of bringing both together into the convergence demonstration and then extending the same agent to 3b.
